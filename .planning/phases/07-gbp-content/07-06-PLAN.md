...
<task type="auto">
  ...
  <files>src/queue/workers/holiday-check.worker.ts, src/scheduler/jobs.ts, src/services/whatsapp/message-handler.ts</files>
  <action>
...
2. Update `src/scheduler/jobs.ts` to add holiday check job:
...
3. Update `src/services/whatsapp/message-handler.ts` to handle hours responses:

```typescript
import { parseHoursResponse } from '../gbp-content/holiday-checker';
import { setSpecialHours, createSingleDayPeriod } from '../google/hours';

// Add hours response detection after post handling, before lead chatbot:

// Check if message looks like hours response
// Pattern: contains date format (DD/MM) and either "סגור" or time
...
if (hoursPattern.test(messageText)) {
  ...
  if (parsedHours.length > 0) {
    console.log(`[message-handler] Parsed ${parsedHours.length} special hours`);
    ...
    if (google) {
      ...
      try {
        ...
        console.error('[message-handler] Failed to update hours:', error);
        await sendWhatsAppMessage(
          tenantId,
          senderPhone,
...
Add a comment in message-handler.ts documenting the priority order:
```typescript
/**
 * Message Processing Priority:
 * 1. Photo responses (if active photo request)
 * 2. Hours responses (holiday special hours)
 * 3. Post responses (monthly post flow)
 * 4. Review approval responses
 * 5. Lead chatbot flow
 */
```
  </action>
