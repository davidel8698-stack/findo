...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/services/review-request/completion.ts"
      provides: "Review completion detection with outcome tracking"
  ...
  key_links:
    ...
    - from: "src/services/review-request/completion.ts"
      to: "src/services/optimization/ab-testing.ts"
      via: "outcome tracking on review completion"
...
<task type="auto">
  <name>Task 3: Integrate A/B testing with review request messages and completion tracking</name>
  <files>src/services/review-request/messages.ts, src/services/review-request/completion.ts</files>
  <action>
**Part A: Update messages.ts for variant selection**
...
export async function composeReviewRequestMessage(
  ...
  return `...existing default template...`;
}
```

**Part B: Update completion.ts for outcome tracking**

Update `src/services/review-request/completion.ts` to record A/B test outcomes:

1. Import recordOutcome:
```typescript
import { recordOutcome } from '../optimization/ab-testing';
```

2. In `checkReviewCompletion` function, after marking a request as completed (around line 58), add outcome tracking:
```typescript
// After: await db.update(reviewRequests).set({ status: 'completed', ... })

// Record A/B test outcome - review request was successful
await recordOutcome(tenantId, 'review_request_message', true);
```

This captures successful outcomes when a review is received after a request was sent.

**Note:** Failure outcomes (request expired without review) should be tracked separately in a future scheduled job that checks for expired requests. For now, we track the success path which is the critical feedback signal.
  </action>
  <verify>
1. Variant selection in messages: `grep -n "getActiveVariant" src/services/review-request/messages.ts`
2. Test type used: `grep -n "review_request_message" src/services/review-request/messages.ts`
3. Outcome tracking in completion: `grep -n "recordOutcome" src/services/review-request/completion.ts`
4. Import exists in completion: `grep -n "ab-testing" src/services/review-request/completion.ts`
  </verify>
  <done>
Review request messages use A/B test variant if assigned, falls back to default template. Completion tracking calls recordOutcome(tenantId, 'review_request_message', true) when a review is received after a request.
  </done>
</task>
...
4. Outcome tracking wired: grep for "recordOutcome" in completion.ts
...
- Completion tracking records success outcomes via recordOutcome
</success_criteria>
