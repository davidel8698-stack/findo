...
<task type="auto">
  <name>Task 3: Create main dashboard and register route</name>
  <files>src/views/dashboard/main.ts, src/views/index.ts, src/routes/pages.ts</files>
  <action>
Create main dashboard page composing health and stats, and add route.
...
3. Mount dashboard API routes:
- Import dashboardApiRoutes from './api/dashboard'
- Add: pagesRoutes.route('/api/dashboard', dashboardApiRoutes);
  </action>
  <verify>Run `bun run typecheck` - all files compile. Start server and visit /dashboard?tenantId=xxx</verify>
  <done>Dashboard accessible at /dashboard with health and stats display</done>
</task>
