...
auto-tuning.worker.ts updates (weekly summary - Phase 8-07 worker):
1. Import shouldNotify, NotificationType from notification-gate
2. The existing sendAllWeeklySummaries function iterates tenants - update it or the worker to check:
```typescript
// Before sending weekly summary to each tenant
const shouldSend = await shouldNotify(tenantId, NotificationType.WEEKLY_REPORT);
if (!shouldSend) {
  console.log(`[auto-tuning] Skipping weekly summary for tenant ${tenantId} (preference disabled)`);
  continue; // Skip this tenant, continue to next
}
```
...
5. **NOTF-01 WhatsApp-first verification:** Enable all WhatsApp notifications for a test tenant, trigger events (new lead, review), confirm WhatsApp messages are actually sent via WhatsApp API
</verification>
...
- WhatsApp notifications actually send when preferences enabled (NOTF-01)
</success_criteria>
