...
<task type="auto">
  ...
  <action>
Update the existing `StickyCtaBar.tsx` (from Phase 14) to scroll to the hero form when clicked.

Note: StickyCtaBar.tsx already exists with frosted glass styling, iOS safe area support, and md:hidden. If for some reason it doesn't exist, create it first with those features before adding scroll behavior.
...
2. **Hero section placement** (IMPORTANT - clarification):
   - Keep HeroContent as-is (it has CTAGroup for the scroll-down button)
   - Add ConversionSection with id="hero-form" as a SEPARATE element BELOW HeroContent
   - This means HeroContent stays unchanged, and ConversionSection is placed after it within the hero area
...
7. Pass id="hero-form" to the first ConversionSection (the one below HeroContent) so StickyCtaBar can scroll to it.

8. CTA count target: 4-5 CTAs (hero form, after social proof, after pricing, after FAQ). This satisfies ACTION-01 (4-6 CTAs).

9. Keep all existing Phase 14-16 sections in place. DO NOT remove any existing sections.
  </action>
  <verify>
