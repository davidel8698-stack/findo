...
user_setup:
  - service: posthog
    ...
    dashboard_config:
      ...
      - task: "Enable Heatmaps feature"
        location: "PostHog Dashboard -> Data Management -> Autocapture -> Enable Heatmaps"
...
must_haves:
  truths:
    ...
    - "Heatmaps track click positions on page elements"
...
Purpose: Enable full behavioral analytics (ANALYTICS-01 through 06) including session replay, funnel tracking, heatmaps, and A/B testing infrastructure.
...
3. Verify heatmaps are enabled:
   - PostHog heatmaps require `autocapture: true` in the client config (added above)
   - User must also enable heatmaps in PostHog Dashboard:
     - Go to Data Management -> Autocapture -> Enable Heatmaps toggle
   - Note this in the summary as a user_setup step
  </action>
  <verify>
    - `npm run build` completes without errors
    - providers.tsx includes PostHogProvider with autocapture: true
    - layout.tsx includes PostHogPageview wrapped in Suspense
    - client.ts has autocapture: true for heatmap support (ANALYTICS-04)
  </verify>
  <done>PostHog provider and pageview tracker integrated into app with heatmap support</done>
</task>
...
5. Heatmaps ready: autocapture: true in config, user notified to enable in dashboard (ANALYTICS-04)
</verification>
...
- Autocapture enabled for heatmap support
