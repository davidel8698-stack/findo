...
<task type="auto">
  <name>Task 3: Add CTA tracking to ConversionSection</name>
  <files>
    website/components/sections/conversion/ConversionSection.tsx
  </files>
  <action>
Update ConversionSection.tsx to pass proper source to LeadCaptureForm:

1. The ConversionSection already has a `variant` prop. Use this as the source:
   ```typescript
...
4. The sticky CTA bar should also track clicks:
   In StickyCtaBar.tsx, add tracking when user clicks:
   ```typescript
   import { trackCtaClick } from "@/lib/posthog/events";
