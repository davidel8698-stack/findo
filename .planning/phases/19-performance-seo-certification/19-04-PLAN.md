...
<task type="auto">
  <name>Task 1: Add comprehensive reduced motion support</name>
  <files>
    website/app/globals.css
  </files>
  <action>
...
Place this AFTER the existing CSS in globals.css (after tailwind imports).
  ...
  <done>Reduced motion and GPU acceleration CSS utilities added</done>
</task>
...
4. Add contain: layout to prevent layout thrashing:
   ```typescript
   // In ActivityFeed container
   <div ref={containerRef} className="... contain-layout">
   ```
   Or via CSS class in globals.css:
   ```css
