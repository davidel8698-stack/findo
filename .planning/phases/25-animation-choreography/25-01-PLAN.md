...
depends_on: []
files_modified:
  - website/lib/hooks/useReducedMotion.ts
...
must_haves:
  ...
  artifacts:
    - path: "website/lib/hooks/useReducedMotion.ts"
      provides: "Centralized reduced motion detection hook"
...
Output:
- useReducedMotion hook for centralized accessibility detection
- useHeroEntrance hook orchestrating the 7-phase GSAP timeline
...
<task type="auto">
  ...
  <files>website/lib/hooks/useReducedMotion.ts</files>
  <action>
Create a centralized hook for reduced motion detection.

Implementation:
- Use `matchMedia("(prefers-reduced-motion: reduce)")` to detect preference
- Return boolean `reducedMotion` state
- Listen for changes (user can toggle while page is open)
- SSR-safe: default to false on server, hydrate on client
...
Create index.ts barrel export if not exists in lib/hooks/.
  </action>
  <verify>File exists and exports useReducedMotion</verify>
  <done>useReducedMotion hook created with SSR-safe implementation</done>
</task>

<task type="auto">
  <name>Task 2: Create useHeroEntrance hook with GSAP timeline</name>
...
Export from lib/hooks/index.ts.
  ...
  <done>useHeroEntrance hook created with 7-phase GSAP timeline and reduced motion fallback</done>
</task>

<task type="auto">
  <name>Task 3: Add data attributes and wire Hero entrance</name>
